...
namespace combi
{
	...
	{
		this->n = n; //мощность исходного множества (не выше 63)
	...
	{
		this->sn = 0; // текущие кол-во элементов массива
		this->mask = 0; // битовая последовательность
	};
	...
	short subset::getnext() //применяется для получения других подмножеств.
	{
		int rc = -1;
		this->sn = 0;
		if (++this->mask < this->count()) rc = getfirst(); // формирует остальные подмножества
		return rc;
	};
	...
	unsigned __int64 subset::count() //вычислет и возвращает количество элементов булеана заданного множества
	{
