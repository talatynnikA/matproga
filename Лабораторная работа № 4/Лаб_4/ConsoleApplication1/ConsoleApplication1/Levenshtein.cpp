...
int min3(int x1, int x2, int x3) //выбрать минимум из трёх
{
	return std::min(std::min(x1, x2), x3);
}

// -- дистанции Левенштeйна (динамическое программирование)
//Главное отличие - использование массива d(для работы с ним использ-ся макрос DD) для хранения результ. промежуточных вычислений.
